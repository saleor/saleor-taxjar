...
import { getDummyOrderCreatedPayload, getDummyTaxesResponseForCreatedOrder, mockRequest } from "../../utils";
...
describe("api/webhooks/order-created", () => {
  ...
  it("rejects when saleor signature is empty", async () => {
    ...
    const { req, res } = mockRequest("POST", "order_created", "example.com", signature);

    const mockedTaxJarResponseData = getDummyTaxesResponseForCreatedOrder();
    const mockedTaxJarResponse =
    mockedCreateOrderTransaction.mockImplementationOnce(() => {return mockedTaxJarResponseData});
